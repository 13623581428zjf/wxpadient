	...
	<div class="mui-content">
		<div>
			<div class="mui-segmented-control">
				...
				<a href="javascript:;" class="mui-control-item" @click="changeStatus('created',1)" :class='{"mui-active":num==1}'>待付款
					<span></span>
				</a>
				<a href="javascript:;" class="mui-control-item" @click="changeStatus('paid',2)" :class='{"mui-active":num==2}'>待就诊
					<span></span>
				</a>
				<!--<a href="javascript:;" class="mui-control-item" @click="changeStatus('',3)" :class='{"mui-active":num==3}'>待取药
					<span></span>
				</a>-->
			</div>
		</div>
			...
			<div class=" mui-scroll">
				<ul class="mui-table-view mui-table-view-chevron"  v-if='isShow=="Y"'>
					<li v-for="item in lists">
						...
						<div style="padding: 11px 15px;" @tap='$router.push({name:"/inquisition/aboutDetail",query:{consultation_id:item.consultationId}})'>
							...
							<div class="pull-right" style="width: 80%;">
								...
								<p style="font-size: 12px; color: #666;">
									...
									<span style="height: 0.4rem;display: inline-block; vertical-align: middle;">
                                    	<span v-if='item.inquisitionType=="text"'>图文问诊：</span>
									<span v-if='item.inquisitionType=="video"'>视频问诊：</span> {{item.consultationTime|sliceTime}} {{item.weekDay |weekfilters}} {{item.fromTime|areaCut}}-{{item.thruTime|areaCut}}</span>
								</p>
							</div>
						</div>
						<div style="height: .8rem;" class="pd-leftright-px15 bg-fff">
							...
							<span style="float: right; padding-top:6px;">
                                <button type="button" class="mui-btn pay mui-btn-success mui-btn-outlined"  v-if="item.status=='created'" @click='toPay(item.consultationId,item.price)'>立即支付</button> 
...
<script>
	export default {
		...
		methods: {
			//删除订单
			deleteOrder: function(e, item,index) {
				var _this=this
				var btnArray = ['取消', '确定'];
			...
			//取消预约
			deleteMake: function(e, item) {
				var _this=this
				var btnArray = ['取消', '确定'];
				mui.confirm('', '你确定取消预约吗？', btnArray, function(e) {
					if(e.index == 1) {
				//成功
